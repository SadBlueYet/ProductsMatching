...
class Parser:
    ...
    def extract_from_offer_tag(self, elem) -> dict:
        ...
        return {
            ...
            "category_lvl1": None if not category_path else category_path.split('/')[0],
            "category_lvl2": None if not category_path else category_path.split('/')[1],
            "category_lvl3": None if not category_path else category_path.split('/')[2],
            "category_remaining": None if not category_path else "/".join(category_path.split('/')[3:]),
            "features": elem.findtext("features"),
            "raiting_count": elem.findtext("raitingCount"),
            "raiting_value": elem.findtext("raitingValue"),
    ...
    def build_category_path_in_stream(self, category_element):
        ...
        if parent_id:
            ...
            if parent_category is not None:
                return self.build_category_path_in_stream(parent_category) + "/" + category_name
...
for i in parser.parsing_xml():
    pprint(parser.extract_from_offer_tag(i))
    break